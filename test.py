...
# ---- 조건 선택 ----
N0_option = st.selectbox("초기 세균 수 선택", ["작음(100마리)", "보통(1000마리)", "많음(5000마리)"])

N0_dict = {"작음(100마리)":100, "보통(1000마리)":1000, "많음(5000마리)":5000}
N0 = N0_dict[N0_option]


r = st.slider("세균 성장 속도 (r)", 0.1, 1.0, 0.5, 0.1)
d = st.slider("세포 사멸률 (Death rate) (사멸률:어떤 집단 내에서 특정 기간 동안 사망하는 개체의 비율)", 0.0, 0.05, 0.01, 0.001)

t_option = st.radio("배양 시간", ["짧음", "보통", "김"])
t_max = {"짧음(30시간)":30, "보통(60시간)":60, "김(100시간)":100}[t_option]
